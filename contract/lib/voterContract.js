...
class MyAssetContract extends Contract {
  ...
  async init(ctx) {
    ...
    let repVotable = await new VotableItem(ctx, 'INC', ballotData.fedDemocratBrief);
    let demVotable = await new VotableItem(ctx, 'BJP', ballotData.republicanBrief);
    let indVotable = await new VotableItem(ctx, 'NCP', ballotData.greenBrief);
    let grnVotable = await new VotableItem(ctx, 'SHS', ballotData.independentBrief);
    let libVotable = await new VotableItem(ctx, 'MNS', ballotData.libertarianBrief);
